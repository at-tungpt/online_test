<table>
    ...
    <tr>
        ...
        <td>
            ...
            <a href="{{ route('student-block', [$user->id]) }}" class="glyphicon glyphicon-ok">{{ trans('label_trans.unlock')}}</a>
            @else
            <a href="{{ route('student-block', [$user->id]) }}" class="glyphicon glyphicon-remove">{{ trans('label_trans.lock')}}</a>
            @endif
            @endif
            @if ( $user->role_id == config('define.ROLETEACHER'))
            @if ($user->status == config('define.DISABLED') )
            <a href="{{ route('teacher-block', [$user->id]) }}" class="glyphicon glyphicon-ok">{{ trans('label_trans.unlock')}}</a>
            @else
            <a href="{{ route('teacher-block', [$user->id]) }}" class="glyphicon glyphicon-remove">{{ trans('label_trans.lock')}}</a>
            @endif
            @endif
            <a href="{{ route('admin-delete', [$user->id]) }}" alt="{{ trans('user_trans.are_you_sure_delete_user') }}" class="delete-user" ><i class="glyphicon glyphicon-trash"></i>{{ trans('label_trans.delete')}}</a>
        </td>
    </tr>
    @endforeach
    </tbody>
